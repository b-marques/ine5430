	...
	int win_sequence = 0;
	  ...
	  for(auto j = 1; j < GRID_SIZE; ++j) {
	    for(auto i_aux = 14, j_aux = j; j_aux < GRID_SIZE; --i_aux, ++j_aux){
	      ...
	      for(auto i_sequence = i_aux, j_sequence = j_aux, count = 0;
	               count < 5; --i_sequence, ++j_sequence, ++count) {
	        if(j_sequence > GRID_SIZE_M_1 || _grid[i_sequence][j_sequence] == -1){
	          diagonal1 = -1;
	          break;
	        }
	        diagonal1 = (diagonal1 << 1) + _grid[i_sequence][j_sequence];
	  ...
	  for(auto i = 1; i < GRID_SIZE; ++i) {
	    for(auto i_aux = i, j_aux = 0; i_aux < GRID_SIZE; ++i_aux, ++j_aux){
	      ...
	      for(auto i_sequence = i_aux, j_sequence = j_aux, count = 0;
	               count < 5; ++i_sequence, ++j_sequence, ++count) {
	        if(i_sequence > GRID_SIZE_M_1 || _grid[i_sequence][j_sequence] == -1){
	          diagonal2 = -1;
	          break;
	        }
	        diagonal2 = (diagonal2 << 1) + _grid[i_sequence][j_sequence];
...
void GomokuCore::treats_sequence_incremental(int& add_to_grade, int* sequence, int seq_size, int play_position, int turn){
...
int GomokuCore::evaluate_incremental(int x, int y, int (&grid)[GRID_SIZE][GRID_SIZE], int turn){
	int	start_x,
		end_x,
		start_y,
		end_y,
...
int GomokuCore::max_search(int (&state)[GRID_SIZE][GRID_SIZE], int depth, int alpha, int beta, int last_grade)
{
  int new_grade;
...
std::tuple<int,int> GomokuCore::minimax(int depth)
{
  ...
  auto beta = INT32_MAX;
  int new_grade = 0,
      initial_grade = evaluate(_grid, _player_turn, depth);
  ...
  for(auto i = 0; i < GRID_SIZE; ++i){
    for(auto j = 0; j < GRID_SIZE; ++j) {
      if(_grid[i][j] == -1) {
        ...
        auto temp_score = max_search(_grid, depth-1, alpha, beta, new_grade);
        // std::cout << temp_score << std::endl;
        if(temp_score > alpha) {
          alpha = temp_score;
...
void GomokuCore::treats_sequence(int (&grid)[GRID_SIZE][GRID_SIZE],
...
int GomokuCore::evaluate(int (&grid)[GRID_SIZE][GRID_SIZE], int turn, int depth)
{
	int n_unities_open = 0,
		n_doubles_open = 0,
		n_triples_open = 0,
	...
	for (int i = 0; i < GRID_SIZE; ++i) {
		...
		for (int j = 0; j < GRID_SIZE; ++j) {
			...
			treats_sequence(grid, i, j, turn, sequence, n_opens, last_position, n_unities_open,
